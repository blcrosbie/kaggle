#!/usr/bin/env python3
"""Generate a Stage 1 submission using gender-specific weighted logistic ensembles."""
...
def _expert_hyperparams(name: str) -> dict[str, float | int]:
    ...
    if name == "ranking":
        return {"epochs": 240, "learning_rate": 0.05, "l2": 0.0008}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a Stage 1 submission using v8 gender-specific ensembles.")
    parser.add_argument("--data-dir", type=Path, default=Path("data"))
    parser.add_argument("--sample-submission", type=Path, default=Path("data/SampleSubmissionStage1.csv"))
    parser.add_argument("--output", type=Path, default=Path("submissions/stage1_v8.csv"))
...
def main() -> None:
    ...
    overall_rows.sort(key=lambda row: (row["season"], row["examples"]))
    overall_brier = round(
        (
        ...
        / max(sum(row["examples"] for row in overall_rows), 1),
        6,
    )
    ...
    payload = {
        ...
        "model": "v8_gender_specific_logistic_ensembles",
