#!/usr/bin/env python3
"""Generate a Stage 1 submission using conference-tournament-aware men ensembles."""
...
def _men_hyperparams(name: str) -> dict[str, float | int]:
    ...
    if name == "tournament_plus":
        return {"epochs": 210, "learning_rate": 0.056, "l2": 0.0012}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a Stage 1 submission using conference-tournament-aware men ensembles.")
    parser.add_argument("--data-dir", type=Path, default=Path("data"))
    parser.add_argument("--sample-submission", type=Path, default=Path("data/SampleSubmissionStage1.csv"))
    parser.add_argument("--output", type=Path, default=Path("submissions/stage1_v19.csv"))
...
def main() -> None:
    ...
    payload = {
        ...
        "model": "v19_conf_tourney_men_nested_calibrated_women",
