#!/usr/bin/env python3
"""Generate a Stage 1 submission using mixed logistic + boosted-stump men experts."""
...
def _weight_vectors(n: int, step: int = 10) -> list[list[float]]:
    buckets = 100 // step
...
def _fit_men_weights(examples: list[TrainingExample], eval_start_season: int) -> tuple[dict[str, float], dict[str, Any]]:
    ...
    for season in seasons:
        ...
        eff_model = train_logistic_model(
            train_rows,
            feature_names=V19_MEN_EXPERTS["efficiency_plus"],
            **v19_params("efficiency_plus"),
        )
        tour_model = train_logistic_model(
            train_rows,
            feature_names=V19_MEN_EXPERTS["tournament_plus"],
            **v19_params("tournament_plus"),
        )
        conf_log_model = train_logistic_model(
            train_rows,
            feature_names=V19_MEN_EXPERTS["conference_tourney"],
            **v19_params("conference_tourney"),
        )
        conf_stump_model = train_boosted_stump_model(
            train_rows,
            feature_names=STUMP_FEATURES,
            n_estimators=48,
            learning_rate=0.07,
            sample_features=6,
            seed=season,
        )
        preds_by_season[season] = {
            "efficiency_plus": [eff_model.predict_prob(example.features) for example in test_rows],
            "tournament_plus": [tour_model.predict_prob(example.features) for example in test_rows],
            "conference_tourney": [conf_log_model.predict_prob(example.features) for example in test_rows],
            "conference_tourney_stumps": [conf_stump_model.predict_prob(example.features) for example in test_rows],
        }
        targets_by_season[season] = [example.label for example in test_rows]

    expert_names = ["efficiency_plus", "tournament_plus", "conference_tourney", "conference_tourney_stumps"]
    candidates = _weight_vectors(len(expert_names), step=10)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a Stage 1 submission using mixed logistic + boosted-stump men experts.")
    parser.add_argument("--data-dir", type=Path, default=Path("data"))
    parser.add_argument("--sample-submission", type=Path, default=Path("data/SampleSubmissionStage1.csv"))
    parser.add_argument("--output", type=Path, default=Path("submissions/stage1_v27.csv"))
...
def main() -> None:
        ...
        if gender == "men":
            ...
            for expert_name in expert_names:
                ...
                if key not in model_cache:
                    ...
                    if expert_name == "conference_tourney_stumps":
                        model_cache[key] = train_boosted_stump_model(
                            train_rows,
                            feature_names=STUMP_FEATURES,
                            n_estimators=48,
                            learning_rate=0.07,
    ...
    payload = {
        ...
        "model": "v27_mixed_logistic_stump_v19_men_nested_calibrated_women",
