#!/usr/bin/env python3
"""Generate a Stage 1 submission using location-aware gender-specific ensembles."""
...
def _expert_hyperparams(name: str) -> dict[str, float | int]:
    if "efficiency" in name:
        return {"epochs": 260, "learning_rate": 0.05, "l2": 0.001}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a Stage 1 submission using v11 location-aware ensembles.")
    parser.add_argument("--data-dir", type=Path, default=Path("data"))
    parser.add_argument("--sample-submission", type=Path, default=Path("data/SampleSubmissionStage1.csv"))
    parser.add_argument("--output", type=Path, default=Path("submissions/stage1_v11.csv"))
...
def main() -> None:
    ...
    payload = {
        ...
        "model": "v11_location_gender_specific_ensembles",
