#!/usr/bin/env python3
"""Generate a Kaggle submission using a pure-Python boosted stump ensemble."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a submission using boosted stumps.")
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("submissions/stage1_boosted_stumps.csv"),
    )
    parser.add_argument(
        "--manifest",
        type=Path,
...
def main() -> None:
    ...
    for game_id, season, team_a, team_b, gender in rows:
        ...
        if key not in models:
            ...
            models[key] = train_boosted_stump_model(train_rows, feature_names=FEATURE_NAMES)
