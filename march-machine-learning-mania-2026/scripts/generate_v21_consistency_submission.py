#!/usr/bin/env python3
"""Generate a Stage 1 submission using conference-tourney plus consistency men ensembles."""
...
def _men_hyperparams(name: str) -> dict[str, float | int]:
    ...
    return {"epochs": 240, "learning_rate": 0.052, "l2": 0.001}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a Stage 1 submission using conference-tourney plus consistency men ensembles.")
    parser.add_argument("--data-dir", type=Path, default=Path("data"))
    parser.add_argument("--sample-submission", type=Path, default=Path("data/SampleSubmissionStage1.csv"))
    parser.add_argument("--output", type=Path, default=Path("submissions/stage1_v21.csv"))
...
def main() -> None:
    ...
    payload = {
        ...
        "model": "v21_conf_tourney_consistency_men_nested_calibrated_women",
