#!/usr/bin/env python3
"""Generate a Stage 1 submission using rigorously nested calibration."""
...
def _evaluate_gender_nested(
    ...
    for season in seasons:
        ...
        for example in test_rows:
            ...
            if gender == "men":
                base_pred = _blend(probs, [weights[name] for name in expert_names])
            else:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a Stage 1 submission using v17 nested calibration.")
    parser.add_argument("--data-dir", type=Path, default=Path("data"))
    parser.add_argument("--sample-submission", type=Path, default=Path("data/SampleSubmissionStage1.csv"))
    parser.add_argument("--output", type=Path, default=Path("submissions/stage1_v17.csv"))
...
def main() -> None:
    ...
    payload = {
        ...
        "model": "v17_nested_calibrated_v12_men_v10_women",
