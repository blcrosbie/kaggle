#!/usr/bin/env python3
"""Generate a Stage 1 submission using v19 experts with fine-grained men weight search."""
...
def _weight_vectors(n: int, step: int = 1) -> list[list[float]]:
    buckets = 100 // step
...
def _fit_men_weights(examples: list[TrainingExample], eval_start_season: int) -> tuple[dict[str, float], dict[str, Any]]:
    ...
    candidates = _weight_vectors(len(expert_names), step=1)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a Stage 1 submission using v19 experts with fine men weight search.")
    parser.add_argument("--data-dir", type=Path, default=Path("data"))
    parser.add_argument("--sample-submission", type=Path, default=Path("data/SampleSubmissionStage1.csv"))
    parser.add_argument("--output", type=Path, default=Path("submissions/stage1_v28.csv"))
...
def main() -> None:
    ...
    payload = {
        ...
        "model": "v28_fine_weight_v19_men_nested_calibrated_women",
