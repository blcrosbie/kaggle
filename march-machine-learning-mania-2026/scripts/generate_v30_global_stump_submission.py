#!/usr/bin/env python3
"""Generate a Stage 1 submission using v19 men experts plus a global boosted-stump expert."""
...
def _fit_men_weights(examples: list[TrainingExample], eval_start_season: int) -> tuple[dict[str, float], dict[str, Any]]:
    ...
    for season in seasons:
        ...
        stump_model = train_boosted_stump_model(
            train_rows,
        ...
        season_preds["global_stumps"] = [stump_model.predict_prob(example.features) for example in test_rows]
        preds_by_season[season] = season_preds
        targets_by_season[season] = [example.label for example in test_rows]

    expert_names = ["efficiency_plus", "tournament_plus", "conference_tourney", "global_stumps"]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a Stage 1 submission using v19 men experts plus global boosted stumps.")
    parser.add_argument("--data-dir", type=Path, default=Path("data"))
    parser.add_argument("--sample-submission", type=Path, default=Path("data/SampleSubmissionStage1.csv"))
    parser.add_argument("--output", type=Path, default=Path("submissions/stage1_v30.csv"))
...
def main() -> None:
    ...
    payload = {
        ...
        "model": "v30_global_stump_v19_men_nested_calibrated_women",
