#!/usr/bin/env python3
"""Generate a Stage 1 submission using folded interaction gender-specific ensembles."""
...
def _expert_hyperparams(name: str) -> dict[str, float | int]:
    if "efficiency" in name:
        return {"epochs": 270, "learning_rate": 0.05, "l2": 0.001}
    if "tournament" in name:
        return {"epochs": 240, "learning_rate": 0.056, "l2": 0.0012}
    if "matchup" in name:
        return {"epochs": 230, "learning_rate": 0.054, "l2": 0.001}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a Stage 1 submission using v10 folded-interaction ensembles.")
    parser.add_argument("--data-dir", type=Path, default=Path("data"))
    parser.add_argument("--sample-submission", type=Path, default=Path("data/SampleSubmissionStage1.csv"))
    parser.add_argument("--output", type=Path, default=Path("submissions/stage1_v10.csv"))
...
def main() -> None:
    ...
    payload = {
        ...
        "model": "v10_folded_interaction_gender_specific_ensembles",
