#!/usr/bin/env python3
"""Generate a Stage 1 submission using conference-tourney plus opponent-quality men ensembles."""
...
def _men_hyperparams(name: str) -> dict[str, float | int]:
    ...
    if name == "conference_tourney":
        return {"epochs": 220, "learning_rate": 0.055, "l2": 0.001}
    return {"epochs": 230, "learning_rate": 0.052, "l2": 0.0009}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a Stage 1 submission using conference-tourney plus opponent-quality men ensembles.")
    parser.add_argument("--data-dir", type=Path, default=Path("data"))
    parser.add_argument("--sample-submission", type=Path, default=Path("data/SampleSubmissionStage1.csv"))
    parser.add_argument("--output", type=Path, default=Path("submissions/stage1_v20.csv"))
...
def main() -> None:
    ...
    payload = {
        ...
        "model": "v20_conf_tourney_quality_men_nested_calibrated_women",
