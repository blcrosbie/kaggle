#!/usr/bin/env python3
"""Generate a Stage 1 submission using richer men-only ranking consensus features."""
...
def _men_hyperparams(name: str) -> dict[str, float | int]:
    ...
    if name == "ranking_consensus":
        return {"epochs": 240, "learning_rate": 0.05, "l2": 0.001}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a Stage 1 submission using v12 ranking-consensus ensembles.")
    parser.add_argument("--data-dir", type=Path, default=Path("data"))
    parser.add_argument("--sample-submission", type=Path, default=Path("data/SampleSubmissionStage1.csv"))
    parser.add_argument("--output", type=Path, default=Path("submissions/stage1_v12.csv"))
...
def main() -> None:
    ...
    payload = {
        ...
        "model": "v12_men_ranking_consensus_v10_women",
